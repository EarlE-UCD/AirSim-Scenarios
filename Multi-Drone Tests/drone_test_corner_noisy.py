...
############ Go to the Corner Test (noisy version) ############
print('Press "c" to fly to the Corner Test (noisy version)')
while True:
...
############ Perform Corner Test (noisy version) ############
print('Press "c" to start the Corner Test (noisy version)')
while True:
    if keyboard.is_pressed('c'):
        break
time.sleep(0.5)
print('Performing Corner Test (noisy version)...')
...
while (x_coordinate1 < -90):
    ...
    pose1 = client.simGetVehiclePose(vehicle_name="Drone1")
    x_coordinate1 = pose1.position.x_val + (4*rand() - 2)
    y_coordinate1 = pose1.position.y_val + (4*rand() - 2)
    z_coordinate1 = pose1.position.z_val + (4*rand() - 2)

    pose2 = client.simGetVehiclePose(vehicle_name="Drone2")
    x_coordinate2 = pose2.position.x_val + (4*rand() - 2)
    y_coordinate2 = pose2.position.y_val + (4*rand() - 2)
    z_coordinate2 = pose2.position.z_val - 15 + (4*rand() - 2)
    
    if (np.sqrt( (x_coordinate1 - x_coordinate2)**2 + (y_coordinate1 - y_coordinate2)**2 + (z_coordinate1 - z_coordinate2)**2) < 4):
        
        print("Close drone detected!")

        test2_1 = client.moveByVelocityAsync(0, 0, -3.8, 1, vehicle_name="Drone1")
        test2_2 = client.moveByVelocityAsync(0, 0, 3.8, 1, vehicle_name="Drone2")
        test2_1.join()
        test2_2.join()

...
############ Go to the Maze Test (noisy version) ############
print('Press "c" to fly to the Maze Test (noisy version)')
while True:
...
############ Perform Maze Test (noisy version) ############
print('Press "c" to start the Maze Test (noisy version)')
while True:
    if keyboard.is_pressed('c'):
        break
time.sleep(0.5)
print('Performing Maze Test (noisy version)...')
...
while (y_coordinate1 < 116):
    ...
    front_distance_sensor_data = client.getDistanceSensorData(distance_sensor_name="DistanceFront", vehicle_name="Drone1")
    distance_from_front = front_distance_sensor_data.distance + (2*rand() - 1)
    
    left_distance_sensor_data = client.getDistanceSensorData(distance_sensor_name="DistanceLeft", vehicle_name="Drone1")
    distance_from_left = left_distance_sensor_data.distance + (2*rand() - 1)
    
    right_distance_sensor_data = client.getDistanceSensorData(distance_sensor_name="DistanceRight", vehicle_name="Drone1")
    distance_from_right = right_distance_sensor_data.distance + (2*rand() - 1)
    ...
    if (direction_code == 0):
        if (distance_from_front < 6):
            ...
            x_checkpoint.append(temporary_x + (3*rand() - 1.5))
            y_checkpoint.append(temporary_y + (3*rand() - 1.5))
            z_checkpoint.append(temporary_z + (3*rand() - 1.5))

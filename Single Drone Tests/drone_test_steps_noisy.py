...
############ Go to the Steps Test (noisy version) ############
print('Press "c" to fly to the Steps Test (noisy version)')
while True:
...
############ Perform Steps Test (noisy version) ############
print('Press "c" to start the Steps Test (noisy version)')
while True:
    if keyboard.is_pressed('c'):
        break
time.sleep(0.5)
print('Performing Steps Test (noisy version)...')

pose = client.simGetVehiclePose(vehicle_name="Drone1")
x_coordinate = pose.position.x_val

while (x_coordinate < 95):
